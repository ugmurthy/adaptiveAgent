...
import { modalityMarker, runStudy } from './jev-routing-study.js';
...
describe('JEV routing study', () => {
  ...
  it('replays exact and approximate history with disagreement, low-confidence, cost, and latency metrics', async () => {
    ...
    const results = await runStudy({
      ...
      runIds: ['execution-exact', 'execution-approx'],
      repeat: 1,
...
async function createFixture(): Promise<{ settingsPath: string; databasePath: string; cachePath: string }> {
  ...
  insertRun(database, 'execution-exact', {
  ...
  insertRun(database, 'execution-approx', {
...
function insertRun(
  ...
  database.query('INSERT INTO agent_runs VALUES (?, ?, ?, ?)').run(id, JSON.stringify(record), createdAt, updatedAt);
}
