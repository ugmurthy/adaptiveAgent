  ...
  const result = await client.query<{
    ...
    select
      ...
      case
        ...
        else coalesce(max(r.status), 'unknown')
      end as status,
      coalesce(
        jsonb_agg(
          jsonb_build_object(
            'rootRunId', r.root_run_id::text,
            'runId', r.id::text,
            'status', r.status,
            'startedAt', r.created_at,
            'completedAt', r.completed_at,
...
async function detectTraceSupport(client: PostgresClient): Promise<{ hasTraceView: boolean; hasToolObservabilityColumns: boolean; hasRunModelColumns: boolean; hasGatewaySessionTables: boolean }> {
  const [viewResult, columnResult, runModelColumnResult, gatewayTablesResult] = await Promise.all([
    ...
    client.query<{ count: string }>(`
      ...
      where table_schema = 'public'
        and table_name in ('gateway_sessions', 'gateway_session_run_links')
    `),
  ]);
  return {
    ...
    hasGatewaySessionTables: Number(gatewayTablesResult.rows[0]?.count ?? 0) >= 2,
  };
}

async function loadSessionOverview(client: PostgresClient, sessionId: string, hasGatewaySessionTables: boolean): Promise<SessionOverview | null> {
  if (!hasGatewaySessionTables) {
    return loadCoreSessionOverview(client, sessionId);
  }
...
async function loadCoreSessionOverview(client: PostgresClient, sessionId: string): Promise<SessionOverview | null> {
  const result = await client.query<{
    id: string;
    status: string;
    current_run_id: string | null;
    ...
    `
      select
        ...
        (array_agg(r.id::text order by r.updated_at desc, r.created_at desc))[1] as current_run_id,
        (array_agg(r.root_run_id::text order by r.updated_at desc, r.created_at desc))[1] as current_root_run_id,
        (array_agg(r.root_run_id::text order by r.completed_at desc nulls last, r.created_at desc) filter (where r.completed_at is not null))[1] as last_completed_root_run_id,
...
async function loadTraceSessionRootRunIds(
  ...
  if (hasGatewaySessionTables) {
    ...
    ordered.push(...result.rows.map((row) => row.root_run_id));
  }
...
async function resolveTraceTarget(
  ...
  if (options.sessionId) {
    const session = await loadSessionOverview(client, options.sessionId, hasGatewaySessionTables);
    const rootRunIds = await loadTraceSessionRootRunIds(client, options.sessionId, session, hasGatewaySessionTables, options.rootRunId);
    return {
      target: {
        kind: 'session',
...
async function loadRootRuns(
  client: PostgresClient,
  rootRunIds: string[],
  sessionId?: string,
  hasRunModelColumns = true,
  hasGatewaySessionTables = true,
): Promise<RootRun[]> {
  if (rootRunIds.length === 0) {
    return [];
  }
  ...
  if (!hasGatewaySessionTables) {
    const result = await client.query<{
      ...
      `
        ...
        select
          ...
          rr.root_run_id::text as run_id,
          'run' as invocation_kind,
          null::integer as turn_index,
    ...
    return rootRunRowsToRootRuns(result.rows);
  }
...
function rootRunRowsToRootRuns(rows: Array<{
  root_run_id: string;
  run_id: string;
  invocation_kind: string;
  turn_index: number | null;
