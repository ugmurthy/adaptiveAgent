...
export async function resolveAgentSdkConfigWithSources(options: AgentSdkOptions): Promise<ResolvedAgentSdkConfigWithSources> {
  ...
  const config: ResolvedAgentSdkConfig = {
    ...
    inference: {
      mode: options.inferenceMode ?? settings.inference?.mode ?? (provider === 'ollama' ? 'local' : 'byok'),
      tier: options.inferenceTier ?? settings.inference?.tier ?? 'medium',
    },
    gateway: {
      url: expandOptional(options.gateway?.url ?? settings.gateway?.url, env),
