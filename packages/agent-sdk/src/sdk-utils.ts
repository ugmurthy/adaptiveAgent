...
export function resolvePath(cwd: string, value: string, env: NodeJS.ProcessEnv = process.env): string { const expanded = expandEnvironmentVariables(value, env); return isAbsolute(expanded) ? resolve(expanded) : resolve(cwd, expanded); }
