...
const INFERENCE_TIERS = ['low', 'medium', 'high', 'xtra-high'] as const;
const WEEKDAYS = ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'] as const;

const ajv = new Ajv({ allErrors: true, allowUnionTypes: true });
const agentValidator = ajv.compile({ type: 'object', required: ['id', 'name', 'invocationModes', 'defaultInvocationMode', 'model', 'tools'], additionalProperties: true, properties: {
  $schema: { type: 'string', nullable: true }, version: { type: 'integer', const: 1, nullable: true },
  id: { type: 'string', minLength: 1 }, name: { type: 'string', minLength: 1 }, description: { type: 'string', nullable: true },
  ...
  recovery: { type: 'object', nullable: true, additionalProperties: false, properties: { continuation: { type: 'object', nullable: true, additionalProperties: false, properties: { enabled: { type: 'boolean', nullable: true }, defaultStrategy: { type: 'string', enum: ['auto', 'same_run', 'resume', 'retry', 'continue'], nullable: true }, requireUserApproval: { type: 'boolean', nullable: true } } }, retryableErrorCodes: { type: 'array', items: { type: 'string' }, nullable: true }, fallbackModels: { type: 'array', nullable: true, items: { type: 'object', required: ['provider', 'model'], additionalProperties: false, properties: { provider: { type: 'string', minLength: 1 }, model: { type: 'string', minLength: 1 }, whenFailureClass: { type: 'array', items: { type: 'string' }, nullable: true }, whenErrorCode: { type: 'array', items: { type: 'string' }, nullable: true } } } } } },
...
const settingsValidator = ajv.compile({ type: 'object', additionalProperties: true, properties: { agent: { type: 'object', additionalProperties: true, nullable: true, properties: { mode: { type: 'string', enum: ['fixed', 'auto'], nullable: true }, configPath: { type: 'string', minLength: 1, nullable: true }, id: { type: 'string', minLength: 1, nullable: true } } }, agentSelection: { type: 'object', additionalProperties: false, nullable: true, properties: { engine: { type: 'string', enum: ['agent', 'typesafe'], nullable: true }, agent: { type: 'string', minLength: 1, nullable: true }, typesafe: { type: 'object', additionalProperties: false, nullable: true, properties: { model: { type: 'string', minLength: 1, nullable: true }, apiKeyEnv: { type: 'string', minLength: 1, nullable: true }, baseUrl: { type: 'string', minLength: 1, nullable: true }, timeoutMs: { type: 'integer', minimum: 1, nullable: true }, policyPath: { type: 'string', minLength: 1, nullable: true }, policy: { type: 'object', additionalProperties: false, nullable: true, properties: { relevance: { type: 'object', additionalProperties: false, nullable: true, properties: { instructions: {}, criteria: { type: 'object', additionalProperties: false, nullable: true, properties: { true: {}, false: {} } } } }, selection: { type: 'object', additionalProperties: false, nullable: true, properties: { instructions: {}, candidateCriteria: {} } }, routing: { type: 'object', additionalProperties: false, nullable: true, properties: { modeInstructions: {}, primaryInstructions: {}, assignmentInstructions: {} } }, minimumRelevance: { type: 'number', minimum: 0, maximum: 1, nullable: true }, minimumConfidence: { type: 'number', minimum: 0, maximum: 1, nullable: true } } } } } } }, executionRouting: { type: 'object', additionalProperties: false, nullable: true, properties: { mode: { type: 'string', enum: ['single', 'adaptive'], nullable: true }, maxSpecialists: { type: 'integer', minimum: 1, maximum: 16, nullable: true }, lowConfidenceFallback: { type: 'string', enum: ['direct', 'error'], nullable: true } } }, runtime: { type: 'object', additionalProperties: true, nullable: true, properties: { mode: { type: 'string', enum: ['memory', 'sqlite', 'postgres'], nullable: true }, autoMigrate: { type: 'boolean', nullable: true }, sqlitePath: { type: 'string', minLength: 1, nullable: true } } }, logging: { type: 'object', additionalProperties: true, nullable: true, properties: { destination: { type: 'string', enum: ['console', 'file', 'both'], nullable: true } } }, inference: { type: 'object', additionalProperties: true, nullable: true, properties: { mode: { type: 'string', enum: INFERENCE_MODES, nullable: true }, tier: { type: 'string', enum: INFERENCE_TIERS, nullable: true } } }, taskPreparation: { type: 'object', additionalProperties: false, nullable: true, properties: { mode: { type: 'string', enum: ['never', 'auto', 'always'], nullable: true }, agent: { type: 'string', minLength: 1, nullable: true }, showPreparedTask: { type: 'boolean', nullable: true } } }, gateway: { type: 'object', additionalProperties: true, nullable: true, properties: { url: { type: 'string', minLength: 1, nullable: true }, clientName: { type: 'string', minLength: 1, nullable: true }, clientVersion: { type: 'string', minLength: 1, nullable: true }, accessTokenEnv: { type: 'string', minLength: 1, nullable: true }, requireRunPermit: { type: 'boolean', nullable: true }, connectTimeoutMs: { type: 'integer', minimum: 1, nullable: true }, requestTimeoutMs: { type: 'integer', minimum: 1, nullable: true }, reconnectAttempts: { type: 'integer', minimum: 0, nullable: true }, remoteTools: { type: 'array', uniqueItems: true, items: { enum: ['web_search', 'read_web_page'] }, nullable: true } } } } });
...
export function validateSettings(value: unknown, path: string): AgentSettingsFile {
  ...
  if (settings.taskPreparation?.mode && settings.taskPreparation.mode !== 'never' && !settings.taskPreparation.agent) {
    throw new AgentSettingsValidationError(path, ['settings.taskPreparation.agent is required when task preparation is enabled']);
  }
  if (settings.agentSelection?.typesafe?.policy && settings.agentSelection.typesafe.policyPath) {
    throw new AgentSettingsValidationError(path, ['settings.agentSelection.typesafe.policy and policyPath cannot both be set']);
  }
  if (settings.agentSelection?.engine === 'typesafe' && !settings.agentSelection.typesafe) {
    throw new AgentSettingsValidationError(path, ['settings.agentSelection.typesafe is required when agentSelection.engine is "typesafe"']);
  }
  if (
    settings.agent?.mode === 'auto'
    && settings.agentSelection?.engine !== 'typesafe'
    && !settings.agentSelection?.agent
    && !settings.taskPreparation?.agent
  ) {
    throw new AgentSettingsValidationError(path, ['settings.agentSelection.agent or settings.taskPreparation.agent is required when settings.agent.mode is "auto"']);
  }
