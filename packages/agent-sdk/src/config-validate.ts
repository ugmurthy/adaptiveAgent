...
export function validateSettings(value: unknown, path: string): AgentSettingsFile {
  ...
  if (settings.agent?.mode === 'auto' && !settings.taskPreparation?.agent) {
    throw new AgentSettingsValidationError(path, ['settings.taskPreparation.agent is required when settings.agent.mode is "auto"']);
  }
