...
export class DurableEventProjector {
  ...
  async projectBatch(limit=100):Promise<number> {
    ...
    let count=0;
    for(const row of [...candidates.rows,...jobs.rows]) { const wake=await this.insert(row);if(wake){count++;await this.bus?.publish(wake);} }
    return count;
  }
  private async insert(row:any):Promise<EventWakeup|undefined> {
    ...
    try { await client.query('begin');await client.query('select id from service_jobs where id=$1 for update',[row.job_id??row.id]);
      const core=row.run_id!==undefined;const jobId=core?row.job_id:row.id;
      ...
      const data=core?publicCoreData(row):jobData(row);
      await client.query('insert into service_public_events(id,job_id,sequence,event_type,data,occurred_at,source_event_id) values($1,$2,$3,$4,$5,$6,$7)',[crypto.randomUUID(),jobId,sequence,core?row.event_type:'job.state_changed',JSON.stringify(data),core?row.created_at:row.updated_at,source]);
