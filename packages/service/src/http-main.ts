...
async function main(): Promise<void> {
  ...
  const app=await buildHttpServer({sdk,authenticate,artifacts:artifactRuntime.manager,eventBus,ensureActor,ready:async()=>{try{await pool.query('select 1');return true;}catch{return false;}},rateLimit:positiveInt(env.HTTP_RATE_LIMIT,100)});
