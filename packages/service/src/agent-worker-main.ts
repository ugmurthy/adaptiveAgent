...
export async function agentWorkerMain(): Promise<void> {
  ...
  const executor = new AgentSdkWorkloadExecutor(bootstrap, registry, new LocalWorkspaceManager(process.env.JOB_WORKSPACE_ROOT ?? './var/jobs',new ArtifactWorkspacePolicy(artifactRuntime.manager)), positiveInt(process.env.MAX_SUBTASKS, 4));
