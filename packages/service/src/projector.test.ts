...
describe('public event projection',()=>{
  ...
  it('assigns monotonic sequences and does not duplicate source events',async()=>{
    ...
    const artifact={projection_kind:'artifact',id:'artifact-1',job_id:'job-1',run_id:'run-1',tool_execution_id:'tool-1',original_filename:'result.txt',media_type:'text/plain',byte_size:'4',content_hash:'hash',status:'available',created_at:'2026-01-01T00:00:00.000Z',available_at:'2026-01-01T00:00:01.000Z',expires_at:'2026-01-08T00:00:01.000Z',updated_at:'2026-01-01T00:00:01.000Z',source_event_id:'artifact:artifact-1:available'};
    const query=async(sql:string)=>sql.includes('from service_job_run_links')?{rows:inserted.some(row=>row.source==='core:42')?[]:[core],rowCount:1}:sql.includes('from service_jobs j left join')?{rows:inserted.some(row=>row.source==='job:state-1')?[]:[job],rowCount:1}:sql.includes('from service_artifacts')?{rows:inserted.some(row=>row.source===artifact.source_event_id)?[]:[artifact],rowCount:1}:{rows:[],rowCount:0};
    ...
    await expect(projector.projectBatch()).resolves.toBe(3);await expect(projector.projectBatch()).resolves.toBe(0);
    expect(inserted.map(row=>[row.sequence,row.source])).toEqual([[1,'core:42'],[2,'job:state-1'],[3,'artifact:artifact-1:available']]);
    expect(inserted[0].data).toEqual({runId:'run-1',coreSequence:7,stepId:'step-1',toolName:'search'});
    expect(inserted[2].data).toMatchObject({artifactId:'artifact-1',filename:'result.txt',status:'available'});
    expect(JSON.stringify(inserted[2].data)).not.toContain('storage');expect(wakeups).toHaveLength(3);
  });
});
