...
<section class="center-card chat-view">
  <div class="view-heading">
    <div><span>Chat · {chat.pinnedAgentName}</span><h2><button class="title-trigger" title={chat.title} aria-label={`View full chat description: ${chat.title}`} on:click={() => onshowtitle('chat', chat.title)}>{chat.title}</button></h2><p>Session {chat.sessionId.slice(0,8)} · {chat.occupied?'Turn in progress':'Ready'}</p></div>
