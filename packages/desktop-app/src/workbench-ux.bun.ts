...
describe('workbench UX decisions', () => {
  ...
  it('labels executable continuation honestly', () => {
    expect(recoveryActionLabel({runId:'r',status:'failed',action:'continue_new_run',executable:true,reason:'retry'})).toBe('Continue in new run');
  });
});
