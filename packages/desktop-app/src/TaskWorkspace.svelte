<script lang="ts">
  ...
  function exportJson() {
    download(JSON.stringify(output(result), null, 2) ?? 'null', 'json', 'application/json;charset=utf-8');
  }
...
<section class="center-card task-view">
  <div class="view-heading task-header">
    ...
    {#if result !== undefined}<details class="export-menu"><summary>Export</summary><div><button on:click={exportJson}>Run output (.json)</button><button on:click={exportMarkdown}>Final result (.md)</button></div></details>{/if}
  </div>
  ...
  {#if !selectedRun.occupiesSlot && result !== undefined}
    <div class="report-tabs">{#each ['result','artifacts'] as tab}<button class:active={reportTab===tab} on:click={() => reportTab=tab as typeof reportTab}>{tab}</button>{/each}</div>
    <div class="completed-report">
      {#if reportTab==='result'}<ResultRenderer value={displayedResult}/>
      {:else if artifacts.length}<ArtifactList {artifacts}/>{:else}<div class="empty-state"><strong>No structured artifacts</strong><p>The result did not include recognizable files or an artifacts/files array.</p></div>{/if}
    </div>
