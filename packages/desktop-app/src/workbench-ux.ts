...
export interface ResultArtifact { name: string; detail?: string }
...
export function extractResultArtifacts(value: unknown): ResultArtifact[] {
  ...
  if (unwrapped && typeof unwrapped === 'object') {
    ...
    return list.flatMap((item) => {
      ...
      return typeof name === 'string' ? [{ name, detail: typeof entry.type === 'string' ? entry.type : undefined }] : [];
    });
  }
  ...
  return [...new Set(matches.map((match) => match.trim().replace(/^[`'"(]|[`'"),.:;]$/g, '')))].map((name) => ({ name }));
}

export function recoveryActionLabel(plan?: RunRecoveryPlan): string {
  if (!plan) return '';
  return ({ resume_same_run: 'Resume run', retry_same_run: 'Retry run', continue_new_run: 'Continue in new run' } as Record<string, string>)[plan.action] ?? '';
}
